...
export class wsRouter {
  ...
  async upgradeHandler(
    ...
    const routes = (
      isSecure === "all"
        ? [
            ...this._router.getRoutes(request.url as string, "wss"),
            ...this._router.getRoutes(request.url as string, "ws"),
          ]
        : this._router.getRoutes(request.url as string, isSecure ? "wss" : "ws")
    ) as callbackFunction[];
    if (routes.length === 0) {
