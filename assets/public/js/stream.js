  ...
  if (userStreams.type === 0) {
      ...
      for (const streamIndex in responce) {
        ...
        if (!stream?.deleted) {
          ...
          if (
            ...
            deleteA.addEventListener("click", async function deleteBrodcast() {
              const responce = await db.deleteBrodcast(localStorage.token, id);
              if (responce.type === 0) {
                streamDiv.remove();
              } else {
                await db.logout();
    ...
    if (id) {
      if (typeof thisStream === "undefined" || thisStream?.deleted === true) {
        ...
        videoNode.parentNode.replaceChild(errorNode, videoNode);
      } else {
        // Show stream
        const manifestUrl = `${apiV1RestBase}/stream/${username}/${
          id || responce.length
